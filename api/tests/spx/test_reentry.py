...
def _ceiling():
    # 5872.40 anchor at 23:00 prev day, +1.05/hr.
    return _line("CHANNEL_CEILING", 5872.40, 23, 1.05)


def _floor():
    # 5848.20 anchor at 17:00 prev day, +1.05/hr (ascending channel).
    return _line("CHANNEL_FLOOR", 5848.20, 17, 1.05)
...
def test_above_armed_without_qualifying_candle():
    ...
    # Ceiling at 09:00 today = 5872.40 + 10 * 1.05 = 5882.90
    bar = Candle(
...
def test_above_triggered_by_bearish_touch_and_close_above():
    # Bearish candle that touches ceiling (5882.90 at 09:00) and closes above.
    bar = Candle(
        t=datetime(2026, 5, 8, 9, tzinfo=CT),
        o=5890.00, h=5891.00, l=5882.50, c=5884.00,  # bearish (c < o), low <= 5882.90 <= high, c > 5882.90
    )
    w = evaluate_reentry("ABOVE_ASCENDING", bar, _ceiling(), _floor())
    assert w.active is True
    assert w.side == "BUY_FROM_ABOVE"
...
def test_below_triggered_by_bullish_touch_and_close_below():
    ...
    # Floor at 09:00 today = 5848.20 + 16 * 1.05 = 5865.00
    bar = Candle(
        t=datetime(2026, 5, 8, 9, tzinfo=CT),
        o=5860.00, h=5867.00, l=5859.00, c=5863.00,  # bullish, touched floor (5865 in range), c<5865
    )
