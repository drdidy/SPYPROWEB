...
def _quote_body(es_last: float, spx_last: float) -> dict:
    return {
        "data": {
            "items": [
                {"symbol": "/ES", "last": es_last, "bid": es_last - 0.25, "ask": es_last + 0.25},
...
def test_fetch_sync_quote_returns_offset(mock_post, mock_get):
    ...
    with patch.dict(os.environ, VALID_ENV, clear=False):
        quote = TastytradeFetcher().fetch_sync_quote()
...
def test_fetch_sync_quote_unavailable_when_symbol_missing(mock_post, mock_get):
    ...
    mock_get.return_value = _ok_response(
        {"data": {"items": [{"symbol": "/ES", "last": 5860.5}]}}
    )
    with patch.dict(os.environ, VALID_ENV, clear=False):
        with pytest.raises(FetcherUnavailable, match="missing /ES or SPX"):
            TastytradeFetcher().fetch_sync_quote()
