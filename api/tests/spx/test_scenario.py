...
def test_inside_when_price_between_active_swing_lines():
    assert fan_zone(5872.0, _projected()) == "BETWEEN_CEILINGS"
    assert classify("ASCENDING", 5872.0, _projected()) == "INSIDE_ASCENDING"


def test_above_when_price_above_both_swing_high_lines():
    assert fan_zone(5901.0, _projected()) == "ABOVE_BOTH_CEILINGS"
    assert classify("ASCENDING", 5901.0, _projected()) == "ABOVE_ASCENDING"


def test_below_when_price_below_both_swing_low_lines():
    assert fan_zone(5820.0, _projected()) == "BELOW_HIGH_FLOOR"
...
def test_direction_argument_no_longer_controls_classification():
    assert classify("DESCENDING", 5872.0, _projected()) == "INSIDE_ASCENDING"
    assert classify("NONE", 5872.0, _projected()) == "INSIDE_ASCENDING"
...
def test_between_ceiling_plays_sell_high_ceiling_or_buy_low_ceiling():
    ...
    assert plays.alternate.entry_line == "PREV_RTH_LOW_ASC"
    assert plays.alternate.exit_line == "PREV_RTH_HIGH_ASC"


def test_above_both_ceilings_uses_high_ceiling_for_buy_support():
    plays = build_plays("ABOVE_ASCENDING", _projected())
    assert plays.primary is not None and plays.alternate is not None
    assert plays.primary.side == "BUY"
    assert plays.primary.entry_line == "PREV_RTH_HIGH_ASC"
    assert plays.primary.exit_price > plays.primary.entry_price
    assert plays.alternate.side == "SELL"
    assert plays.alternate.entry_line == "PREV_RTH_HIGH_ASC"
    assert plays.alternate.exit_line == "PREV_RTH_HIGH_DESC"


def test_below_both_ceilings_uses_low_ceiling_sell_or_high_floor_buy():
    plays = build_plays("ABOVE_DESCENDING", _projected(high_desc=5858.0))
    assert plays.primary is not None and plays.alternate is not None
    assert plays.primary.side == "SELL"
    assert plays.primary.entry_line == "PREV_RTH_LOW_ASC"
    assert plays.primary.exit_line == "PREV_RTH_HIGH_DESC"
    assert plays.alternate.side == "BUY"
    assert plays.alternate.entry_line == "PREV_RTH_HIGH_DESC"
    assert plays.alternate.exit_line == "PREV_RTH_LOW_ASC"


def test_below_high_floor_uses_low_floor_buy():
    plays = build_plays("BELOW_DESCENDING", _projected())
    assert plays.primary is not None and plays.alternate is not None
    assert plays.primary.side == "BUY"
...
def test_unfavorable_play_prices_are_suppressed():
    ...
    assert plays.primary is not None
    assert plays.alternate is None


def test_fan_read_names_professional_zone():
    read = build_fan_read(5872.0, _projected())
    assert read.zone == "BETWEEN_CEILINGS"
    assert read.label == "Between fan ceilings"
    assert read.primary_reference == "PREV_RTH_HIGH_ASC"


def test_explain_inside_mentions_pivot_fan():
    text = explain_scenario("INSIDE_ASCENDING", 5872.0, _projected())
    assert "High Fan Ceiling" in text
    assert "Low Fan Ceiling" in text
