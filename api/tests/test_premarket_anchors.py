...
def _ct_index(rows: list[tuple[str, float, float, float, float]]) -> pd.DataFrame:
    """rows: [(iso_no_tz, open, high, low, close), ...] in CT."""
...
def _bear(o: float, l: float) -> tuple[float, float, float, float]:
    """o, h, l, c for a red candle of width o->c, with low at l."""
    c = o - 0.50
    h = o + 0.05
    return o, h, min(l, c) - 0.01, c


def _bull(o: float, h: float) -> tuple[float, float, float, float]:
    c = o + 0.60
    l = o - 0.05
...
def test_no_qualifying_candle_returns_empty():
    # All bullish - nothing bearish to anchor.
    rows = [
    ...
    df = _ct_index(rows)
    out = pc.find_premarket_anchors(df, date(2026, 5, 7))
...
def test_primary_picks_lowest_lower_low_with_green_follower():
    # 5am makes a lower low than 4am AND 6am closes green -> 5am qualifies.
    # 4am does not have a lower low than 3am.
    rows = [
        ("2026-05-07 02:00", 580.0, 580.5, 579.8, 580.3),                # bull baseline
        ("2026-05-07 03:00", *(580.3, 580.5, 579.7, 580.0)),             # red, low 579.7
        ("2026-05-07 04:00", *(580.0, 580.2, 579.6, 579.85)),            # red, low 579.6, lower than 3am OK but follower 5am is RED
        ("2026-05-07 05:00", *(579.85, 580.0, 579.0, 579.40)),           # red, low 579.0, lower than 4am OK
        ("2026-05-07 06:00", *_bull(579.40, 580.20)),                    # green follower for 5am
    ...
    # 4am is bearish but its follower (5am) is red, so it's only a secondary.
    assert out["anchor2"] is None
...
def test_anchor2_added_only_when_7am_lower_and_qualifies():
    # Primary at 5am (low 579.5, follower 6am green).
    # 7am bearish with low 578.0 (lower than 5am) and 8am green follower
    # -> anchor2 = 7am.
    rows = [
        ("2026-05-07 02:00", *_bull(580.0, 580.4)),
        ("2026-05-07 03:00", *_bull(580.4, 580.8)),
        ("2026-05-07 04:00", 580.5, 580.9, 580.10, 580.20),               # red, low 580.10
        ("2026-05-07 05:00", 580.20, 580.30, 579.50, 579.80),             # red, lower-low OK
        ("2026-05-07 06:00", *_bull(579.80, 580.50)),                     # green follower
        ("2026-05-07 07:00", 580.50, 580.55, 578.00, 578.20),             # red, lower than primary
        ("2026-05-07 08:00", *_bull(578.20, 579.00)),                     # green follower
    ]
    df = _ct_index(rows)
    out = pc.find_premarket_anchors(df, date(2026, 5, 7))
    assert out["primary"].timestamp.hour == 5
...
def test_anchor2_skipped_if_7am_not_lower_than_primary():
    rows = [
        ...
        ("2026-05-07 05:00", 580.20, 580.30, 579.50, 579.80),             # primary low 579.50
        ("2026-05-07 06:00", *_bull(579.80, 580.50)),
        ("2026-05-07 07:00", 580.50, 580.55, 579.80, 580.00),             # red but low > primary
...
def test_red_with_red_follower_disqualifies_lowest_low():
    # 5am has the lowest low BUT 6am closes red (not green). 5am must NOT
    # qualify as primary. 6am also fails (its low isn't lower than 5am).
    # Result: no qualified primary; both bearish bars surface as secondaries.
    rows = [
        ("2026-05-07 02:00", *_bull(720.0, 720.5)),
        ("2026-05-07 03:00", *_bull(720.4, 720.8)),
        ("2026-05-07 04:00", 720.5, 720.7, 720.10, 720.20),               # red, low 720.10
        ("2026-05-07 05:00", 720.20, 720.30, 716.46, 717.00),             # red, lower-low OK
        ("2026-05-07 06:00", 717.00, 717.20, 716.80, 716.90),             # red follower -> disqualifies 5am
...
def test_user_example_6am_anchor_with_5am_lower_secondary():
    # Real example user described:
    #   5am red, low 716.46
    #   6am red, low below 5am AND 7am closes green -> 6am is the primary anchor.
    #   5am still appears as secondary even though it didn't have a green follower.
    rows = [
        ("2026-05-07 02:00", *_bull(720.0, 720.5)),
        ("2026-05-07 03:00", *_bull(720.4, 720.8)),
        ("2026-05-07 04:00", *_bull(720.6, 721.0)),
        ("2026-05-07 05:00", 720.80, 720.90, 716.46, 717.00),             # red, low 716.46
        ("2026-05-07 06:00", 717.00, 717.10, 716.20, 716.40),             # red, low 716.20 (< 5am)
        ("2026-05-07 07:00", *_bull(716.40, 720.00)),                     # green follower
        ("2026-05-07 08:00", *_bull(720.00, 721.50)),
    ]
    df = _ct_index(rows)
    out = pc.find_premarket_anchors(df, date(2026, 5, 7))
...
def test_anchor_lines_returns_three_descending_lines_at_offset():
    ...
    lines = pc.build_anchor_lines(anchor, slope=0.20)
    assert len(lines) == 3
    upper, main, lower = lines
    assert upper.zone_type == "CALL_ZONE"
    assert main.zone_type == "MAIN"
    assert lower.zone_type == "PUT_ZONE"
    assert upper.anchor_price == 580.0 + pc.ANCHOR_BAND_OFFSET
    assert main.anchor_price == 580.0
    assert lower.anchor_price == 580.0 - pc.ANCHOR_BAND_OFFSET
    for ln in lines:
...
def test_anchor_lines_descend_at_slope_to_9am():
    ...
    # 4 hours after 5am at slope 0.20 descending = 580.0 - 0.80 = 579.20
...
def _line_at(value: float, ts: pd.Timestamp) -> pc.DynamicLine:
    """Flat line equal to `value` at time `ts` (slope 0 for predictable tests)."""
...
def test_buy_trigger_red_touch_close_above_within_band():
    ...
    # red candle, low 579.50 (touches), close 580.50 (above by 0.50, within 1.7)
    row = pd.Series({"Open": 581.00, "High": 581.20, "Low": 579.50, "Close": 580.50})
    assert pc.is_anchor_buy_trigger(row, line, ts) is True
...
def test_buy_trigger_rejects_when_close_too_far_above_line():
    ...
    ts = pd.Timestamp("2026-05-07 10:00", tz=ct)
    line = _line_at(580.00, ts)
    # red, low 579.50, close 582.00 (above by 2.0, exceeds 1.7)
    row = pd.Series({"Open": 583.00, "High": 583.10, "Low": 579.50, "Close": 582.00})
    assert pc.is_anchor_buy_trigger(row, line, ts) is False
...
def test_buy_trigger_rejects_when_wick_does_not_touch():
    ...
    ts = pd.Timestamp("2026-05-07 10:00", tz=ct)
    line = _line_at(580.00, ts)
    # red, low 580.30 - never touched line
...
def test_sell_trigger_green_touch_close_below_within_band():
    ...
    ts = pd.Timestamp("2026-05-07 10:00", tz=ct)
    line = _line_at(580.00, ts)
    # green, high 580.50 (touches), close 579.50 (below by 0.50, within 1.7)
    row = pd.Series({"Open": 579.00, "High": 580.50, "Low": 578.80, "Close": 579.50})
    assert pc.is_anchor_sell_trigger(row, line, ts) is True
...
def test_sell_trigger_rejects_when_close_too_far_below_line():
    ...
    ts = pd.Timestamp("2026-05-07 10:00", tz=ct)
    line = _line_at(580.00, ts)
    # green, high 580.50, close 578.00 (below by 2.0, exceeds 1.7)
    row = pd.Series({"Open": 577.00, "High": 580.50, "Low": 576.80, "Close": 578.00})
    assert pc.is_anchor_sell_trigger(row, line, ts) is False
...
def test_detect_anchor_triggers_emits_one_per_touched_line():
    ct = pc.get_central_tz()
    rows = [
        ("2026-05-07 09:00", 580.50, 581.00, 580.20, 580.80),  # green, doesn't touch line at 580
        ("2026-05-07 10:00", 581.20, 581.30, 579.80, 580.40),  # red, touches 580 from above, closes above by 0.40 OK
        ("2026-05-07 11:00", 580.40, 580.50, 580.10, 580.20),  # red, doesn't touch
    ]
    df = _ct_index(rows)
    line = _line_at(580.00, df.index[0])
    triggers = pc.detect_anchor_triggers(df, [line])
...
def test_anchor_open_zone_categorizes_price():
    ...
    lines = pc.build_anchor_lines(anchor, slope=0.20)
    # At 9am, descending 0.80 from anchor: Main=579.20, Upper=582.60, Lower=575.80.
    assert pc.anchor_open_zone(583.50, lines, ts_9) == "ABOVE_UPPER"
    assert pc.anchor_open_zone(581.00, lines, ts_9) == "UPPER_TO_MAIN"
    assert pc.anchor_open_zone(577.00, lines, ts_9) == "MAIN_TO_LOWER"
    assert pc.anchor_open_zone(575.00, lines, ts_9) == "BELOW_LOWER"
