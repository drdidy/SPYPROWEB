...
export function ChannelStateRail({
  ...
  const stateLabel = PHASE_DEFINITIONS[current]?.label ?? formatEngineStateLabel(current);
  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="text-right font-mono text-[11px] tracking-[0.02em] text-ink-3">
          <div>{stateLabel}</div>
          {nextEventISO && (
            <div className="mt-1 text-[10px] uppercase tracking-[0.12em] text-ink-2">
          ...
          return (
            ...
            >
              ...
              <span
                aria-hidden
                className={cn(
                  "mx-auto mb-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full font-mono text-[10px] font-bold",
                  isCurrent
              ...
              <span className="absolute right-1 top-1 opacity-70">
                <InfoTooltip
                  label={phase.label}
                  content={`${phase.summary} Enter: ${phase.enterOn} Exit: ${phase.exitOn}`}
                  placement="bottom"
                />
              </span>
            </li>
